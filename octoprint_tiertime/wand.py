...
class tiertime_printer_status_item:
    ...
    motorPositionY, motorPositionZ, nNozzleNum, nTargetPlatTemp, nTargetTemp1, nTargetTemp2, nozzleTemp1, nozzleTemp2, 
    printerStatus, printing, remainPercent, remainTime, strPrinterStatus) :
        self._sn = str(SN)        
...
class wandServer:
            ...
            if len(usbList) > 0 :
                for usbPrinter in usbList :
                    printer_item = tiertime_printer_item(str(usbPrinter["serNum"]),"USB", False, 
                            usbPrinter["printName"], usbPrinter["printIcon"], usbPrinter["systemType"], str(0))                    
            ...
            if len(netList) > 0 :
                for netPrinter in netList :
                    printer_item = tiertime_printer_item(str(netPrinter["serNum"]),"Ethernet", False, 
                            netPrinter["printName"], netPrinter["printType"], netPrinter["systemType"], str(netPrinter["accessCtrl"]))
    ...
    def uploadfile(self, sn, filename, printTimes = 1):
        ...
        self.send_command("sf:{\"cmd\": \"uploadfile\",\"fileName\":\""+fname+"\",\"taskqueue\":0,\"printcount\":"+str(printTimes)+"}")
    ...
    def cb_uploadfile(self, obj) :        
        ...
        if status == 0:
            ...
            if filepath is not None:
                self.send_command("{\"cmd\":\"exec\",\"sn\":\""+self._upload_sn+"\",\"action\":\"sendtaskfile\",\"filepaths\":[\""+filepath+"\"],\"username\":\""+self._identifier+"\"}")
    ...
    def cb_printmodel(self, obj) :
        self._logger.info("cb_printmodel :" + json.dumps(obj, sort_keys=True, indent=4, separators=(',', ': ')))
    ...
    def get_extruder_temperature(self, sn, extruderIndex) -> float:
        ...
        if t_status is not None:
            ...
            if temp > 0 and temp < 10:
                temp = float(getattr(t_status, "nTargetTemp" + str(extruderIndex+1), 0.0)) * temp
            return temp
        return -1
    ...
    def	cb_getallprintstatus(self, obj) :
        ...
        for n in range(0, len(statusArray)) :
            ...
            printer_status_item = tiertime_printer_status_item(serNum, statusArray[n]["basePlatTemp"], statusArray[n]["curHeight"], statusArray[n]["curLayer"],
                ...
                statusArray[n]["motorPositionY"], statusArray[n]["motorPositionZ"], 
                statusArray[n]["nNozzleNum"], statusArray[n]["nTargetPlatTemp"], statusArray[n]["nTargetTemp1"], statusArray[n]["nTargetTemp2"], 
                statusArray[n]["nozzleTemp1"], statusArray[n]["nozzleTemp2"],
                statusArray[n]["printerStatus"], statusArray[n]["printing"], statusArray[n]["remainPercent"], statusArray[n]["remainTime"], statusArray[n]["strPrinterStatus"])
